...
class VllmScheduler(Scheduler):
    ...
    _last_scheduler_cls: type | None = (
        None  # what get_scheduler_cls() resolved to, for the self-check
    )
    ...
    def _admit(self, current):
        ...
        while self._arrivals and self._arrivals[0].arrival <= current:
            ...
            prompt = (
                list(req.input_hash_ids)
                if req.input_hash_ids
                else [req.id * (1 << 20) + i for i in range(req.input)]
            )
            vreq = VllmRequest(
    ...
    def _refresh_views(self):
        ...
        self.waiting = self._arrivals + [self._sim[r.request_id] for r in self._core.waiting]
    ...
    def _build_batch(self, current, sys, out):
        ...
        ) * self.memory.npu_pool.bytes_per_block
        batch = Batch(
            self.get_batch_id(),
            self.model,
            total_len,
    ...
    def _complete_step(self, out, emits, batch, finish):
        ...
        for request_id, tokens in zip(request_ids, sampled):
            ...
            computed_before = (
                req.num_computed_tokens - num_new if req.num_computed_tokens >= num_new else 0
            )
