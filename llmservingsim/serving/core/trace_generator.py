...
def _build_trace_ctx(
    ...
    if not model_type:
        raise KeyError(
            ...
            f"llmservingsim/profiler/models/<model_type>.yaml"
        )
...
def _emit_layer(
    ...
    if category is None:
        raise KeyError(
            ...
            f"llmservingsim/profiler/models/<model_type>.yaml or remove it from the sequence."
        )
...
def _attn_load_balancer(requests, tp_size, pim_channels=0, channel_split=1):
    ...
    for req in requests:
        if req.is_init:
            ...
            # one channel can handle multiple heads if the load is still small
            for _ in range(channel_split):
