from .logger import get_logger, print_markup, print_rule
from .utils import *
...
class PowerModel:
    def __init__(self, power_configs):
        ...
        self.base_powers = [
            {"base_node": 0, "npu": 0, "cpu": 0, "dram": 0, "link": 0, "nic": 0, "storage": 0}
        ...
        for i, power_config in enumerate(self.power_configs):
            ...
            self.base_powers[i]["cpu"] = (
                power_config["cpu"]["idle_power"]
                + (power_config["cpu"]["active_power"] - power_config["cpu"]["idle_power"])
                * power_config["cpu"]["util"]
            )
            ...
            self.base_powers[i]["link"] = (
                power_config["link"]["idle_power"] * power_config["link"]["num_links"]
            )
            self.base_powers[i]["nic"] = (
                power_config["nic"]["idle_power"] * power_config["nic"]["num_nics"]
            )
            self.base_powers[i]["storage"] = (
                power_config["storage"]["idle_power"] * power_config["storage"]["num_devices"]
            )
    ...
    def add_npu_standby_energy_consumption(
        self, hardware, node_id, current_ns, last_end_ns, last_calc_ns, num_npus=1
    ):
        ...
        if end2current_latency_ns <= calc2current_latency_ns:
            ...
            standby_duration_s = self.power_configs[node_id]["npu"][hardware][
                "standby_duration"
            ] - (end2current_latency_ns * 1e-9)  # effective standby duration left
            if standby_duration_s < 0:
                standby_duration_s = 0.0

        standby_energy = (
            self.power_configs[node_id]["npu"][hardware]["standby_power"]
            - self.power_configs[node_id]["npu"][hardware]["idle_power"]
        ) * min(standby_latency_s, standby_duration_s)  # J = W × s
        self.net_energies[node_id]["npu"] += (
            standby_energy * num_npus
        )  # should be total npus in the instance
    ...
    def add_npu_active_energy_consumption(self, hardware, node_id, latency_ns, num_npus=1):
        ...
        # NPU
        energy_j = (
            self.power_configs[node_id]["npu"][hardware]["active_power"]
            - self.power_configs[node_id]["npu"][hardware]["idle_power"]
        ) * latency_s  # J = W × s
        self.net_energies[node_id]["npu"] += (
            energy_j * num_npus
        )  # should be npus in the group (npus running the layer)
        self.npu_log += energy_j * num_npus
        # CPU
        cpu_active_util = max(
            0.7 - self.power_configs[node_id]["cpu"]["util"], 0
        )  # assume max CPU utilization during NPU active time
        energy_j = (
            (
                self.power_configs[node_id]["cpu"]["active_power"]
                - self.power_configs[node_id]["cpu"]["idle_power"]
            )
            * cpu_active_util
            * latency_s
        )  # J = W × s
    ...
    def add_pim_active_energy_consumption(self, node_id, latency_ns):
        latency_s = latency_ns * 1e-9  # ns → s
        energy_j = (
            self.power_configs[node_id]["dram"]["pim_active_power"]
            - self.power_configs[node_id]["dram"]["idle_power"]
        ) * latency_s  # J = W × s
        self.net_energies[node_id]["dram"] += energy_j
        self.dram_log += energy_j
    ...
    def get_current_power(self, current_time_ns):
        ...
        for node_id, net_node_energy in enumerate(self.net_energies):
            total_energy = (
                sum(net_node_energy.values())
                + sum(self.base_powers[node_id].values()) * current_time_s
            )  # J = W × s
            current_power_w += round(
                float(
                    (total_energy - self.last_energies[node_id])
                    / ((current_time_ns - self.last_time_ns) * 1e-9)
                ),
                2,
            )  # W
    ...
    def get_final_energy(self, end_time_ns):
        ...
        for node_id, net_node_energy in enumerate(self.net_energies):
            total_energy = (
                sum(net_node_energy.values()) + sum(self.base_powers[node_id].values()) * end_time_s
            )  # J = W × s
    ...
    def print_power_summary(self):
        for node_id, total_node_energy in enumerate(self.total_energies):
            print_rule()
            print_markup(
                f"Node {node_id} total energy consumption (kJ):                               "
                f"{total_node_energy / 1000:.2f}"
            )
            entries = list(self.net_energies[node_id].items())
            for i, (key, value) in enumerate(entries):
                tree_indent = "└─" if i == len(entries) - 1 else "├─"
                total_energy = value + self.base_powers[node_id][key] * self.end_time_s
                print_markup(
                    f"{tree_indent} {DEVICE_STR[key]} energy consumption (J):"
                    f"{DEVICE_SPACE[key]}{total_energy:.2f}"
                )
            print_rule()
...
def total_ring_data(L_bytes: float, N: int, collective: str = "allreduce"):
    ...
    if coll == "allreduce":
        total_bytes = 2 * (N - 1) * S
    elif coll in ("alltoall", "allgather", "reducescatter"):
        # Single ring pass: each of N-1 steps transfers one chunk per rank.
        total_bytes = (N - 1) * S
    else:
        raise ValueError(
            f"Unknown collective {collective!r}. "
            "Supported: allreduce, alltoall, allgather, reducescatter"
        )
