...
def _load_cluster_config_for_overrides(path):
    with open(_cluster_config_path(path)) as f:
        return json.load(f)
...
def _build_instance_runtime_configs(instances, args, dtype_to_bits):
    ...
    for instance_id, instance in enumerate(instances):
        ...
        if kv_cache_dtype not in ("auto", "fp8"):
            raise ValueError(
                f"Unsupported kv_cache_dtype '{kv_cache_dtype}' for instance {instance_id}"
            )
        ...
        runtime_configs.append(
            {
                ...
                "npu_memory_utilization": _resolve_mem_util(instance, args.npu_memory_utilization),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="python -m llmservingsim.serving", description="LLMServingSim"
    )

    parser.add_argument(
        "--cluster-config",
        type=str,
        default="configs/cluster/single_node_single_instance.json",
    ...
    if any_prefix_caching and enable_prefix_sharing and prefix_storage != "None":
        ...
        if prefix_storage == "CPU":
            for i in range(num_prefix_pool):
                if cpu_mem_size[i] <= 0:
                    raise RuntimeError(
                        f"Memory size for prefix storage type {prefix_storage} is invalid"
                    )
                inst_ids = node2inst_mapping[i]
                prefix_pools.append(
                    build_prefix_pool(
                        pool_device,
                        cpu_mem_size[i] * GB_TO_BYTE,
                        _pool_block_size(inst_ids),
                        _pool_kv_bytes_per_token(inst_ids),
                        node_id=i,
                    )
                )
            ...
            prefix_pools.append(
                build_prefix_pool(
                    pool_device,
                    cluster["cxl_mem_size"] * GB_TO_BYTE,
                    _pool_block_size(inst_ids),
                    _pool_kv_bytes_per_token(inst_ids),
                )
            )
            ...
            raise NotImplementedError(
                f"Prefix storage type {prefix_storage} is not supported or memory size is invalid"
            )
    ...
    for instance_id, instance in enumerate(instances):
        ...
        prefix_pool = None
        if prefix_pool_index is not None:
        ...
        schedulers.append(
            scheduler_cls(
                ...
                instance["node_id"],
                instance_id,
                inst_cfg["max_num_seqs"],
                inst_cfg["max_num_batched_tokens"],
                instance["num_npus"],
                instance["tp_size"],
                instance["pp_size"],
                instance["npu_mem"]["mem_size"],
                cpu_mem_size[instance["node_id"]],
                inst2npu_mapping[instance_id],
                instance["pd_type"],
                inst_cfg["fp"],
                inst_cfg["block_size"],
                num_req,
    ...
    if network_backend == "ns3":
        astra_args.append(
            ...
            + astra_sim
            + "/inputs/logical_topology/logical_8nodes_1D.json"
        )
    p = subprocess.Popen(
        astra_args,
    ...
    dp_pending = {
        dg: defaultdict(deque) for dg in dp_groups
    }  # dg -> {inst: deque[(batch, node_id)]}
    ...
    while True:
        ...
        if power_modeling and sys == inst2npu_mapping[instance_id] and waiting_request[instance_id]:
            power_model.add_npu_standby_energy_consumption(
                instances[instance_id]["hardware"],
                node_id,
                current,
                last_end_time[instance_id],
                last_calc_time[instance_id],
                num_npus=instances[instance_id]["num_npus"],
            )
        ...
        # standing in for.
            ...
            new_req is None
            and instance_id in inst_dp_group
            ...
            if any(dp_pending[dg][i] for i in dp_groups[dg]):
                ...
                logger.debug(
                    f"Instance {instance_id} is idle but DP group {dg} has pending batches. Creating dummy batch for synchronization."
                )
                dummy = Batch(
                    schedulers[instance_id].get_batch_id(),
                    instances[instance_id]["model_name"],
                    1,
                    1,
                    [1],
                    [],
                ...
                if all(dp_pending[dg][i] for i in dp_groups[dg]):
                    ...
                    for inst_id in dp_groups[dg]:
                        ...
                        trace_data = generate_trace(
                            ...
                            inst_cfg["max_num_seqs"],
                            placement[inst_id],
                            block_mode_on[inst_id],
                            expert_routing_policy,
                            inst_cfg["enable_prefix_caching"],
                            inst_cfg["enable_attn_offloading"],
                            power_model,
                            pim_models[nid],
                            inst_cfg["enable_sub_batch_interleaving"],
                            inst_cfg["fp"],
                            dtype=inst_cfg["dtype"],
                            kv_cache_dtype=inst_cfg["kv_cache_dtype"],
                            tp_dim=inst.get("tp_dim"),
                            ep_dim=inst.get("ep_dim"),
                    ...
                    if own_workload is not None:
                        ...
                        controller.write_flush(
                            p, _pass_response(router, current, state_changed=True)
                        )
                    responded = True
                else:
            ...
            if built_here:  # first NPU of the instance, opening a new batch
                ...
                if dg is not None:
                    ...
                    trace_data = generate_trace(
                        ...
                        dtype=inst_cfg["dtype"],
                        kv_cache_dtype=inst_cfg["kv_cache_dtype"],
                        tp_dim=instance["tp_dim"],
                        ep_dim=instance["ep_dim"],
                        enable_block_copy=inst_cfg["enable_block_copy"],
                        step_overhead_us=inst_cfg["step_overhead_us"],
                        prefill_step_overhead_us=inst_cfg["prefill_step_overhead_us"],
                        inputs_root=run_paths.inputs_root,
        ...
        if current > last_log + INTERVAL and sys == inst2npu_mapping[instance_id]:
            ...
            for inst_id in range(num_instances):
                ...
                waiting_reqs = len(
                    [req for req in schedulers[inst_id].waiting if req.arrival <= current]
                )
            ...
            if node2inst_mapping:
                ...
                for i, (node_id, inst_ids) in enumerate(node2inst_mapping.items()):
                    ...
                    if (
                        any_prefix_caching and enable_prefix_sharing and prefix_storage == "CPU"
                    ) or (len(inst_ids) == 1):
                        print_markup(line)
                    else:
                        parts = []
                        for j, inst_cpu_usage in enumerate(inst_usage):
                            inst_cpu_util = (
                                (inst_cpu_usage / node_cpu_usage) * 100 if node_cpu_usage else 0
                            )
        ...
        if (
            (instance_id not in decode_instance or is_prefill_done)
            ...
            if dg is not None:
                all_dp_empty = all(
                    schedulers[inst_id].is_request_empty()
                    and len(schedulers[inst_id].inflight) == 0
                    for inst_id in dp_groups[dg]
                )
                if not all_dp_empty:
        ...
        elif new_req is None and not responded:
    ...
    if any_prefix_caching:
        for i in range(num_instances):
            if not schedulers[i].enable_prefix_caching:
                continue
            (temp_npu_a, temp_npu_b), (temp_cpu_a, temp_cpu_b) = schedulers[
                i
            ].memory.return_prefix_info()
            if (
                (not enable_prefix_sharing)
                and (prefix_storage != "None")
                and (temp_npu_a != temp_cpu_a)
            ):
                raise RuntimeError(
                    f"Instance[{i}] prefix caching requested tokens mismatch between NPU ({temp_npu_a}) and CPU ({temp_cpu_a})"
                )
    ...
    print_markup(
        f"Total latency (s):                                                  {total_latency:.3f}"
    )
    ...
    if total_recompute:
        print_markup(
            f"Recomputed prompt tokens (preemption):                               {total_recompute}"
        )
    print_markup(f"Total generated tokens:                                             {total_gen}")
    print_markup(
        f"Request throughput (req/s):                                         {req_cnt / total_latency:.2f}"
    )
    print_markup(
        f"Average prompt throughput (tok/s):                                  {total_prompt / total_latency:.2f}"
    )
    print_markup(
        f"Average generation throughput (tok/s):                              {total_gen / total_latency:.2f}"
    )
    print_markup(
        f"Total token throughput (tok/s):                                     {(total_prompt + total_gen) / total_latency:.2f}"
    )
    print_markup(
        f"Throughput per {log_interval:g} sec (\\[prompt_throughput], \\[gen_throughput]): {throughput}"
    )
    print_rule()
    if any_prefix_caching:
        print_rule("[sim.tagline]Prefix Caching Results[/]")
        print_markup(
            f"Total requested prompt tokens:                                      {total_requested_tokens}"
        )
        print_markup(
            f"NPU prefix hit prompt tokens:                                       {total_npu_hit_tokens}"
        )
        if total_requested_tokens > 0:
            print_markup(
                f"NPU prefix hit ratio (%):                                           {(total_npu_hit_tokens / total_requested_tokens) * 100:.2f}"
            )
            if prefix_storage != "None":
                print_markup(
                    f"{prefix_storage} prefix hit prompt tokens:                                       {total_cpu_hit_tokens}"
                )
                print_markup(
                    f"{prefix_storage} prefix hit ratio (%):                                           {(total_cpu_hit_tokens / total_requested_tokens) * 100:.2f}"
                )
            print_markup(
                f"Total prefix hit ratio (%):                                         {((total_npu_hit_tokens + total_cpu_hit_tokens) / total_requested_tokens) * 100:.2f}"
            )
        else:
            print_markup(
                "NPU prefix hit ratio (%):                                           N/A (no requests tracked)"
            )
        print_rule()
    if power_modeling:
        ...
        print_markup(
            f"Total energy consumption (kJ):                                      {total_energy / 1000:.2f}"
        )
        # Each node results
