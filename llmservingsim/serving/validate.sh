...
#
#   ./llmservingsim/serving/validate.sh                    both stages (this is the one to run)
#   ./llmservingsim/serving/validate.sh --clocks-only      skip the accuracy stage
#   ./llmservingsim/serving/validate.sh dp moe_dp_pp       only these scenarios, no accuracy stage
#   ./llmservingsim/serving/validate.sh --list             print the scenario names
#   ./llmservingsim/serving/validate.sh --update           rerun and rewrite the baselines
#   ./llmservingsim/serving/validate.sh --update <name>    rewrite just that scenario's baseline
#   ./llmservingsim/serving/validate.sh --help             this text
#
# Two stages:
...
#   2. accuracy  -- regenerates each llmservingsim/bench/examples entry's outputs/sim.csv and
...
for entry in "${SCENARIOS[@]}"; do
    ...
    timeout "$TIMEOUT" "$PYTHON" -m llmservingsim.serving "${COMMON[@]}" "${extra[@]}" \
...
if [[ $ACCURACY -eq 1 ]]; then
    echo
    echo "Regenerating llmservingsim/bench/examples (per-request accuracy vs recorded vLLM runs)..."
    ...
    acc_rc=0
    ./llmservingsim/bench/examples/run.sh > "$LOG_DIR/bench_examples_run.log" 2>&1 || acc_rc=$?
    if [[ $acc_rc -eq 0 ]]; then
        ./llmservingsim/bench/examples/validate.sh > "$LOG_DIR/bench_examples_validate.log" 2>&1 || acc_rc=$?
    fi
    if [[ $acc_rc -ne 0 ]]; then
        echo "Accuracy: FAIL -- llmservingsim/bench/examples exited $acc_rc, see $LOG_DIR/bench_examples_*.log"
        fails=$((fails + 1))
    else
        n_ok=0 n_seen=0
        for csv in llmservingsim/bench/examples/*/*/outputs/sim.csv; do
            ex=${csv#llmservingsim/bench/examples/}; ex=${ex%/outputs/sim.csv}
            ...
            for pair in "accuracy|$csv|sim.csv" "summary|llmservingsim/bench/examples/$ex/validation/summary.txt|summary.txt"; do
...
if [[ $UPDATE -eq 1 ]]; then
    ran_clocks=$(printf '%s\n' "${results[@]}")
    {
        echo "# Total clock counts (ns) for llmservingsim/serving/validate.sh. The simulator is"
        echo "# deterministic, so these are exact. Regenerate with:"
        echo "#     ./llmservingsim/serving/validate.sh --update"
        ...
        # md5 of each llmservingsim/bench/examples sim.csv, the accuracy baseline
        for r in "${results[@]}"; do
