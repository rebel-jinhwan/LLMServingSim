...
def detect_platform() -> PlatformSpec | None:
    ...
    for spec in registry().values():
        try:
            ...
            logger.warning(
                "platform %r is_available() raised %s: %s", spec.name, type(e).__name__, e
            )
            ok = False
        if ok:
    ...
    *,
    ...
    if chosen not in reg:
        raise ValueError(
            ...
            f"llmservingsim/platforms/, or installed through the {ENTRY_POINT_GROUP!r} entry-point group)"
        )
    return reg[chosen]
