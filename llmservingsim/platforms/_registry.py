...
def _builtin_specs() -> list[PlatformSpec]:
    ...
    for mod in sorted(pkgutil.iter_modules(platforms.__path__), key=lambda m: m.name):
        if not mod.ispkg:
            continue
        modname = f"{platforms.__name__}.{mod.name}"
        try:
        ...
        for obj in vars(module).values():
            if (
                inspect.isclass(obj)
                and issubclass(obj, PlatformSpec)
                and obj is not PlatformSpec
...
def _plugin_specs() -> list[PlatformSpec]:
    ...
    for ep in sorted(entry_points(group=ENTRY_POINT_GROUP), key=lambda e: (e.name, e.value)):
        try:
            ...
            logger.warning(
                "skipping platform plugin %s = %s: %s: %s", ep.name, ep.value, type(e).__name__, e
            )
    return specs
...
def registry() -> dict[str, PlatformSpec]:
    ...
    for spec in [*_builtin_specs(), *_plugin_specs()]:
        if spec.name in reg:
            logger.warning(
                "ignoring platform %r from %s: the name is already registered by %s",
                spec.name,
                type(spec).__module__,
                type(reg[spec.name]).__module__,
            )
            continue
        reg[spec.name] = spec
    return reg
