...
class RBLNProfile(PlatformProfile):

    # enforce_eager would bypass the compiled graph that is being measured,
    # and vLLM's dummy loader draws its random weights with a torch Generator
    # on the model's device, which torch-rbln does not provide ("Expected a
    # 'cpu' device type for generator but found 'rbln'"), so the real
    # checkpoint is loaded.
    ENGINE_KWARGS = {"enforce_eager": False, "load_format": "auto"}

    # The collectives are inside the compiled graph, so tp<N> is measured on
    # N real ranks; there is nothing for ASTRA-Sim to add.
    TP_EMULATION = False
    ...
    def decode_buckets(self, max_num_seqs: int) -> list[int]:
    ...
    def step_grid(self, args: ProfileArgs, limits: RuntimeLimits) -> Iterator[Shot]:
        """Lone prefills at the padded chunk over the kv_prefill axis, and
        decode batches at each bucket over the kv_decode axis. Never mixed,
        ...
        for n in self.decode_buckets(max(1, limits.max_num_seqs // pp)):
            for kd in kv_vals:
