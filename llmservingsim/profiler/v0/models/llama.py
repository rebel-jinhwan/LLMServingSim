...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.gate_proj = nn.Linear(self.hidden_size, self.intermediate_size // config.tp_size, bias=config.mlp_bias)
        self.up_proj = nn.Linear(self.hidden_size, self.intermediate_size // config.tp_size, bias=config.mlp_bias)
        self.down_proj = nn.Linear(self.intermediate_size // config.tp_size, self.hidden_size, bias=config.mlp_bias)
        self.act_fn = ACT2FN[config.hidden_act]
        self._gate_proj_timer = Timer(name="gate_proj")
        self._up_proj_timer = Timer(name="up_proj")
        self._act_fn_timer = Timer(name="act_fn")
        self._down_proj_timer = Timer(name="down_proj")

    def forward(self, x):
        with self._gate_proj_timer:
            gate = self.gate_proj(x)
        with self._up_proj_timer:
            up = self.up_proj(x)
        with self._act_fn_timer:
            act = self.act_fn(gate) * up
        with self._down_proj_timer:
