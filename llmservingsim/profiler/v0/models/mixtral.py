...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if getattr(self, "collect_router_stats", False) and not self.training:
            with torch.no_grad():
                sel = selected_experts.view(-1) # [B*S, top_k] → [B*S*top_k]
...
class MixtralAttention(nn.Module):
    ...
    def __init__(self, config: MixtralConfig, layer_idx: int):
        ...
        self.is_causal = True
        self.q_proj = nn.Linear(config.hidden_size, config.num_attention_heads * self.head_dim // self.config.tp_size, bias=False)
        self.k_proj = nn.Linear(config.hidden_size, config.num_key_value_heads * self.head_dim // self.config.tp_size, bias=False)
        self.v_proj = nn.Linear(config.hidden_size, config.num_key_value_heads * self.head_dim // self.config.tp_size, bias=False)
        self.o_proj = nn.Linear(config.num_attention_heads * self.head_dim // self.config.tp_size, config.hidden_size, bias=False)
