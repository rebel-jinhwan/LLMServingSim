...
class PhimoeAttention(nn.Module):
    ...
    def __init__(self, config: PhimoeConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim // self.config.tp_size, bias=self.config.attention_bias)
        self.k_proj = nn.Linear(
            self.hidden_size, self.num_key_value_heads * self.head_dim // self.config.tp_size, bias=self.config.attention_bias
        )
        self.v_proj = nn.Linear(
            self.hidden_size, self.num_key_value_heads * self.head_dim // self.config.tp_size, bias=self.config.attention_bias
        )
        self.o_proj = nn.Linear(self.num_heads * self.head_dim // self.config.tp_size, self.hidden_size, bias=self.config.attention_bias)
...
class PhimoeFlashAttention2(PhimoeAttention):
    ...
    def forward(
        ...
        query_states = query_states.view(bsz, q_len, self.num_heads // self.config.tp_size, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, self.num_key_value_heads // self.config.tp_size, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, self.num_key_value_heads // self.config.tp_size, self.head_dim).transpose(1, 2)
...
class PhimoeBlockSparseTop2MLP(nn.Module):
    def __init__(self, config: PhimoeConfig):
        ...
        self._act_fn_timer = Timer(name="act_fn")
...
class PhimoeSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.experts = PhimoeBlockSparseTop2MLP(config)
        ...
        # statistics
        self.collect_router_stats = config.collect_router_stats
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if getattr(self, "collect_router_stats", False) and not self.training:
            with torch.no_grad():
                sel = selected_experts.view(-1)  # [T*K]
                counts = torch.bincount(sel, minlength=self.num_experts)
                print("[RouterStats] MoE token → expert mapping "
                    f"(num_experts={self.num_experts}): {counts.cpu().tolist()}"
                )
...
class PhimoeForCausalLM(PhimoePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size // config.tp_size, bias=self.config.lm_head_bias)
