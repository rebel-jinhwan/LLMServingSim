...
```
bench/                          Python package — `llmservingsim bench ...`
...
`llmservingsim workloads` produces and `llmservingsim serving --dataset`
...
| `--dtype` | `bfloat16` | note: not inferred from the model config, unlike `llmservingsim serving` |
...
`llmservingsim workloads` (ShareGPT, single-turn, vLLM
...
use `bench/bench.sh` (or `llmservingsim bench run`) from inside the vLLM
container.
