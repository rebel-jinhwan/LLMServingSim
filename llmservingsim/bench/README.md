...
```
bench/                          Python package — `python -m llmservingsim.bench ...`
...
`python -m llmservingsim.workloads.generators` produces and `python -m llmservingsim.serving --dataset`
...
| `--dtype` | `bfloat16` | note: not inferred from the model config, unlike `python -m llmservingsim.serving` |
...
`python -m llmservingsim.workloads.generators` (ShareGPT, single-turn, vLLM
...
./llmservingsim/bench/examples/run.sh                       # all four examples
./llmservingsim/bench/examples/run.sh RTXPRO6000/Qwen3-30B-A3B-Instruct-2507   # single example

# Then validate against the committed vLLM artifacts:
./llmservingsim/bench/examples/validate.sh
./llmservingsim/bench/examples/validate.sh RTXPRO6000/Qwen3-30B-A3B-Instruct-2507
```
...
use `bench/bench.sh` (or `python -m llmservingsim.bench run`) from inside the vLLM
container.
