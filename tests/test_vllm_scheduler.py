...
``python -m llmservingsim.serving.core.vllm_scheduler``; needs vLLM importable.
"""
...
def test_vllm_scheduler():
    ...
    rows = [
        json.loads(line) for line in open(f"{repo}/workloads/sharegpt-llama-3.1-8b-300-sps10.jsonl")
    ][:20]
    ...
    def run(cls, mem_util, prefix):
        s = cls(
            ...
            True,
            0,
            npu_memory_utilization=mem_util,
        )
    ...
    for mem_util, prefix in ((0.9, False), (0.9, True), (0.7, False), (0.7, True)):
        ...
        if installed is not UpstreamScheduler:
            ...
            print(
                f"note: {label}: {installed.__module__}.{installed.__name__} ran "
        ...
        if p_pre == 0:
            ...
            print(
                f"ok: {label}: {p_pre} vs {v_pre} preemptions from vLLM's null block; "
