...
def test_plugin_discovery() -> None:
    ...
    fake = [
        ...
        _EntryPoint(
            "broken", "missing_pkg:Spec", error=ImportError("No module named 'missing_pkg'")
        ),
    ...
    try:
        ...
        assert type(reg["cuda"]).__module__ == "llmservingsim.platforms.cuda", (
            "a plugin replaced a built-in"
        )
        joined = "\n".join(warnings)
        for needle in (
            "does not match",
            "not a PlatformSpec subclass",
            "missing_pkg",
            "already registered",
        ):
        ...
        assert device.supports_kv_cache_dtype("auto") and not device.supports_kv_cache_dtype(
            "fp8"
        ), device
        ...
        try:
            ...
            assert (
                resolve_cluster_config("configs/cluster/single_node_single_instance.json")
            ...
            assert (
                resolve_cluster_config("elsewhere/example_one_node.json")
        ...
        _reset()
    print(
        "ok: plugins register through entry points with their own devices/, perf/ and cluster/; "
