...
| `hardware` | string | Hardware tag. Must match `llmservingsim/profiler/perf/{hardware}/` (e.g. `RTXPRO6000`, `H100`) |
...
possible: 14 of the `python -m llmservingsim.serving` flags can be re-specified per
...
When you launch the simulator, `llmservingsim/serving/core/config_builder.py` reads
...
| `rtx4090_tp2_instance.json` | 2x RTX 4090 as `tp=2`, Llama-3.1-8B. Needs an RTX4090 `tp=2` profile first — only `tp1` is bundled, so it raises `FileNotFoundError` as shipped | [Profiler](../llmservingsim/profiler/overview) |
