...
- Installs the Python deps the simulator needs, three of them
  **version-pinned** for the image's Python 3.10:
  ...
  by `llmservingsim/serving/__main__.py`, `rich` by the loggers, `pandas` by the
  scheduler / trace generator / PIM model, `numpy` by the scheduler,
  and `matplotlib` by `llmservingsim/bench/core/plots.py`. If you install by hand,
  keep the pins.
...
That is the exact path `llmservingsim/serving/__main__.py` launches as a subprocess,
...
`scripts/docker-sim.sh` already runs `pip install -e . --no-deps`, which puts
`serving`, `platforms`, `profiler`, `bench` and `workloads` on the import path.
Outside the container, do it once in your environment:

```bash
pip install -e .
```

Nothing is copied: the packages keep running from the checkout, and an
out-of-tree platform plugin then finds LLMServingSim without a `PYTHONPATH`.

Then run the unit checks and the bundled smoke test from inside the simulator
container:
...
Nothing is copied: the packages keep running from the checkout, so `python -m
llmservingsim.serving` works from any working directory without a `PYTHONPATH`.

Then run the bundled smoke test from inside the simulator container:
