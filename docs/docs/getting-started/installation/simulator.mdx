...
Nothing is copied: the packages keep running from the checkout, and you get the
`llmservingsim` command:

```bash
llmservingsim serving --help      # also: profiler, bench, workloads
```

`python -m llmservingsim.serving` runs the same program without installing.
