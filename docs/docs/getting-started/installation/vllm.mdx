...
- **Profile** a new GPU or model (`llmservingsim profiler`)
- **Benchmark** real vLLM end-to-end (`llmservingsim bench run`)
- **Validate** the simulator against vLLM (`llmservingsim bench validate`)
...
- Mounts the repo root to `/workspace` (so `llmservingsim profiler`,
  `llmservingsim bench`, `llmservingsim workloads` all work)
