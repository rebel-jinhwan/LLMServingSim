...
- **Profile** a new GPU or model (`python -m llmservingsim.profiler`)
- **Benchmark** real vLLM end-to-end (`python -m llmservingsim.bench run`)
- **Validate** the simulator against vLLM (`python -m llmservingsim.bench validate`)
- **Generate** ShareGPT-style workload datasets
  (`python -m llmservingsim.workloads.generators`)
...
The Docker path uses the official `vllm/vllm-openai:v0.24.0` image,
...
image line in `scripts/docker-vllm.sh` to `vllm/vllm-openai:v0.24.0-cu130`
...
- Mounts the repo root to `/workspace` (so `python -m llmservingsim.profiler`,
  `python -m llmservingsim.bench`, `python -m llmservingsim.workloads.generators` all work)
...
You should see vLLM `0.24.0` and your GPU listed.
...
- Installs `vllm==0.24.0` (with `VLLM_USE_PRECOMPILED=1` for the
...
You should see vLLM `0.24.0` and your GPU.

**Bare-metal caveats**

**CUDA driver mismatch** is the most common failure mode. The
`vllm==0.24.0` wheel `VLLM_USE_PRECOMPILED=1` fetches is built against
