...
| `--engine-kwargs` | JSON | none | Extra vLLM `EngineArgs` for the vLLM-driven scheduler (`--scheduler vllm` or the `rbln` platform), e.g. `'{"max_model_len": 65536}'`; an error with the in-tree scheduler |
