...
| Intent | Edit |
| --- | --- |
| Add a new hardware target | Run the profiler with `HARDWARE=` set; output lands in `llmservingsim/profiler/perf/<hw>/`. See **[Profiler / Adding hardware](/docs/profiler/adding-hardware)** |
...
`llmservingsim/bench/examples` entry's `sim.csv` and `validation/summary.txt` checked
