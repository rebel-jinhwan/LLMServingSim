...
1. **Behaviour** — every scenario in `llmservingsim/serving/validate-baselines.txt`,
   ...
   profiles.
2. **Accuracy** — regenerates each `llmservingsim/bench/examples` entry's `outputs/sim.csv`
...
2. If a `sim.csv` changed, also run `./llmservingsim/bench/examples/validate.sh` and commit
...
./llmservingsim/bench/examples/run.sh RTXPRO6000/Llama-3.1-8B

# 2. Compare against the committed vLLM reference
./llmservingsim/bench/examples/validate.sh RTXPRO6000/Llama-3.1-8B
```

Output lands in `llmservingsim/bench/examples/RTXPRO6000/Llama-3.1-8B/validation/`:
...
`llmservingsim/bench/examples/<hardware>/<model>/validation/summary.txt`, not against the ~5%
...
> Validation: `./llmservingsim/bench/examples/validate.sh RTXPRO6000/Llama-3.1-8B` →
