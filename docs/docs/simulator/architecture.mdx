...
frontend** (`llmservingsim/serving/`) that does request scheduling, batching, and
...
```mermaid
flowchart LR
    subgraph PY["Python frontend (llmservingsim/serving/)"]
        direction TB
        ROUTER[Router]
        SCHED[Scheduler]
...
| --- | --- | --- | --- |
| **Python frontend** | `llmservingsim/serving/` | Requests, Batches, Traces | Scheduling, prefix caching, memory accounting, trace generation |
...
`llmservingsim/serving/__main__.py` drives the simulation. Every iteration of the
...
`llmservingsim/serving/core/`. Each is the subject of a dedicated page in this
section:
