...
4. Sweeps **dense / per_sequence / attention / moe** shot grids,
   writing CSVs under `llmservingsim/profiler/perf/<HW>/<MODEL>/<variant>/tp<N>/`.
...
| `HARDWARE` | Free-form label that becomes the folder name under `llmservingsim/profiler/perf/`. Pick something meaningful (e.g., `RTXPRO6000`, `H100`, `MI300X`) |
...
| `--out-root` | `llmservingsim/profiler/perf` | — |
...
`llmservingsim/profiler/perf/`, and `--model-config-root` to point at a different
