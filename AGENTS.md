...
│   │   ├── validate.sh                  # every scenario vs recorded clocks + llmservingsim/bench/examples digests
...
├── workloads/                           # Request trace datasets (.jsonl), not packaged
...
```
llmservingsim/profiler/perf/<hw>/<model>/<variant>/tp<N>/*.csv (profiled latencies)
...
- **`pip install -e .`** puts the packages on the import path as
  ...
  point is `python -m llmservingsim.<pkg>`. Code and the architecture yamls live
  in the package; bulk data does not — `configs/`, `workloads/`, `astra-sim/`,
  `scripts/`, `docs/` and `outputs/` stay at the repository root. (The profiled
  CSV bundles under `llmservingsim/profiler/perf/` and the recorded runs under
  `llmservingsim/bench/examples/` are the exception: they are each tool's own
  output, read through a path relative to the package)
...
`llmservingsim/profiler/perf/`. Don't "restore" log space because the sweep is
...
Profile CSV path: `llmservingsim/profiler/perf/<hardware>/<model>/<variant>/tp<N>/{dense,
per_sequence,attention,moe,skew,skew_fit}.csv` (resolved as
`../llmservingsim/profiler/perf/...` from the `astra-sim/` working directory).
...
- `hardware`: must match a directory name in `llmservingsim/profiler/perf/<hardware>/`
...
1. **`./llmservingsim/serving/validate.sh`** — the whole check, ~8 min. Stage 1 compares every
   ...
   `llmservingsim/serving/validate-baselines.txt`; stage 2 regenerates each `llmservingsim/bench/examples`
...
2. For the *size* of an accuracy change, not just its presence:
   `./llmservingsim/bench/examples/validate.sh` regenerates `validation/summary.txt` and the
