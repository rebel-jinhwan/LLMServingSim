...
Request workloads consumed by `llmservingsim serving --dataset <...>` and by
`llmservingsim bench run --dataset <...>`. Static `.jsonl` files live at the
...
│   ├── __main__.py            llmservingsim workloads <name> ...
...
Generated on demand by `llmservingsim workloads sharegpt --model <hf-id>
...
`llmservingsim serving` or `llmservingsim bench run`.
